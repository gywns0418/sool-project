      ...
      <div class="form-shell">
        ...
        <div class="step-bar">
          ...
          <div class="step-item" :class="{ active: step === 3 }">
            ...
            <span class="step-text">비밀번호 변경</span>
          </div>
        </div>
        ...
        <form class="form-body" @submit.prevent>
            ...
            <div class="field-group">
              ...
              <div class="field-inline">
                ...
                >
                  ...
                  <span v-else>아이디 확인</span>
                </button>
              </div>
              <p
                v-if="loginIdMsg"
                class="field-msg"
                :class="{ success: loginIdChecked, error: !loginIdChecked }"
              >
                {{ loginIdMsg }}
              </p>
            </div>
          ...
          <template v-else>
            ...
            <div class="btn-row">
              ...
              >
                <span v-if="loading && loadingType === 'resetPassword'" class="spinner"></span>
                <span v-else>
                  비밀번호 변경하기 <span class="btn-arrow">→</span>
                </span>
              </button>
            </div>
          </template>
...
async function handleExistLoginId() {
  if (!loginId.value) {
    loginIdChecked.value = false
  ...
  setLoading('checkLoginId', '아이디 존재 여부를 확인하고 있습니다.')

  try {
    const res = await checkLoginId({ loginId: loginId.value })

    if (res.data === false || res.data?.available === false) {
      loginIdChecked.value = true
      loginIdMsg.value = '존재하는 아이디입니다.'
    } else {
      loginIdChecked.value = false
      loginIdMsg.value = '존재하지 않는 아이디입니다.'
    }
  } catch (e) {
    loginIdChecked.value = false
    loginIdMsg.value = e.response?.data?.message || '아이디 확인 중 오류가 발생했습니다.'
  } finally {
    clearLoading()
  }
}
...
async function handleSendEmailCode() {
  ...
  if (!loginIdChecked.value) {
    alert('아이디 확인을 먼저 진행하세요.')
    return
  }
...
async function handleResetPassword() {
  ...
  setLoading('resetPassword', '비밀번호를 변경하고 있습니다.')
